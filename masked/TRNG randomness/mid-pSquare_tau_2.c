...
inline uint32_t TRNG_GetRand(void)
{
  ...
  return (RNG->DR & p); // Return only the lower 31 bits
}
...
void __attribute__((always_inline)) SQ_dshares(const uint32_t *a, uint32_t *b)
{
  ...
  for(int j=1; j<d; j++)
  {
    uint32_t r_r = TRNG_GetRand();
    uint32_t r_p = TRNG_GetRand();
...
void __attribute__((noinline)) Encrypt(uint32_t **plaintext, uint32_t **key, const uint32_t* tweak, uint32_t **ciphertext)
{
  ...
  for(int step=0; step<steps; step+=2)
  {
    ...
    // Add tweakey
		for(int i=0; i<4; i++) state[i][0] = AddModMersenne(state[i][0], AddModMersenne(round_tweaks[step+1][i+4], key[i][0]));
    for(int i=1; i<d; i++)
    {
      for(int j=0; j<4; j++)
      {
...
void __attribute__((noinline)) Decrypt(uint32_t **ciphertext, uint32_t **key, const uint32_t* tweak, uint32_t **plaintext)
{
  ...
  for(int step=0; step<steps; step+=2)
  {
    ...
    // Add tweakey
		for(int i=0; i<4; i++) state[i][0] = SubModMersenne(state[i][0], AddModMersenne(round_tweaks[steps-(step+1)][i+4], key[i][0]));
    for(int i=1; i<d; i++)
    {
      for(int j=0; j<4; j++)
      {
