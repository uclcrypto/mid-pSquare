...
#define RAND_CONST_S 0xa7
#define RAND_CONST_L 0x618dde64
...
inline void lfsr3_bs(uint32_t* tk);
...
void precompute_lfsr_tk3(uint32_t* tk, const uint8_t* key);
void permute_tk(uint32_t* tk, const uint8_t* key);
void precompute_tk_masked_share1(uint32_t* rtk, const uint8_t* tk1, const uint8_t* tk2);
void precompute_tk_masked_shares(uint32_t* rtk, const uint8_t* tk);
void skinny128_tk_masked(uint32_t* rtk, const uint8_t* tk1, const uint8_t* tk2);
...
void __attribute__((noinline)) skinny128_masked_encrypt(uint8_t* ctext, const uint8_t* ptext, const uint8_t* tk1, const uint8_t* tk2);
void __attribute__((noinline)) skinny128_masked_decrypt(uint8_t* ptext, const uint8_t* ctext, const uint8_t* tk1, const uint8_t* tk2);
...
void precompute_tk_masked_share1(uint32_t* rtk, const uint8_t* tk1, const uint8_t* tk2)
{
	...
	precompute_lfsr_tk3(rtk, &tk2[16]);
...
void skinny128_tk_masked(uint32_t* rtk, const uint8_t* tk1, const uint8_t* tk2)
{
	precompute_tk_masked_share1(rtk, tk1, tk2);
	for(int d=1; d < N_SHARES; ++d)
	{
		precompute_tk_masked_shares(rtk+4*ROUNDS*d, &tk1[d*16]);
	}
}
...
void __attribute__((noinline)) skinny128_masked_encrypt(uint8_t* ctext, const uint8_t* ptext, const uint8_t* tk1, const uint8_t* tk2)
{
	...
	skinny128_tk_masked(rtk, tk1, tk2);
	...
	QUADRUPLE_ROUND_MASKED(state, rtk+160, tmp_msk)
	QUADRUPLE_ROUND_MASKED(state, rtk+176, tmp_msk)
	QUADRUPLE_ROUND_MASKED(state, rtk+192, tmp_msk)
	QUADRUPLE_ROUND_MASKED(state, rtk+208, tmp_msk)
...
void __attribute__((noinline)) skinny128_masked_decrypt(uint8_t* ptext, const uint8_t* ctext, const uint8_t* tk1, const uint8_t* tk2)
{
	...
	uint32_t tmp_msk[N_SHARES*2];
	INV_QUADRUPLE_ROUND_MASKED(state, rtk+208, tmp_msk)
	INV_QUADRUPLE_ROUND_MASKED(state, rtk+192, tmp_msk)
	INV_QUADRUPLE_ROUND_MASKED(state, rtk+176, tmp_msk)
	INV_QUADRUPLE_ROUND_MASKED(state, rtk+160, tmp_msk)
...
{
	// Test Vector 1
	const uint8_t plaintext0[16]  = {0xa3, 0x99, 0x4b, 0x66, 0xad, 0x85, 0xa3, 0x45, 0x9f, 0x44, 0xe9, 0x2b, 0x08, 0xf5, 0x50, 0xcb};
	const uint8_t key0[16]        = {0xdf, 0x88, 0x95, 0x48, 0xcf, 0xc7, 0xea, 0x52, 0xd2, 0x96, 0x33, 0x93, 0x01, 0x79, 0x74, 0x49};
	const uint8_t tweak0[32]      = {0xab, 0x58, 0x8a, 0x34, 0xa4, 0x7f, 0x1a, 0xb2, 0xdf, 0xe9, 0xc8, 0x29, 0x3f, 0xbe, 0xa9, 0xa5, 0xab, 0x1a, 0xfa, 0xc2, 0x61, 0x10, 0x12, 0xcd, 0x8c, 0xef, 0x95, 0x26, 0x18, 0xc3, 0xeb, 0xe8};
	const uint8_t ciphertext0[16] = {0x94, 0xec, 0xf5, 0x89, 0xe2, 0x01, 0x7c, 0x60, 0x1b, 0x38, 0xc6, 0x34, 0x6a, 0x10, 0xdc, 0xfa};

	// Test Vector 2
	const uint8_t plaintext1[16]  = {0x7c, 0x1e, 0x33, 0x4f, 0x09, 0x2f, 0x58, 0xb5, 0xd7, 0xa0, 0x9c, 0x76, 0xe7, 0x4b, 0x3a, 0x0a};
	const uint8_t key1[16]        = {0x84, 0xc5, 0xa0, 0x95, 0x0b, 0x5b, 0x92, 0x86, 0x92, 0xc9, 0x73, 0x3b, 0x71, 0x1b, 0x33, 0x0c};
	const uint8_t tweak1[32]      = {0x61, 0x2d, 0x3d, 0x25, 0x3e, 0xc2, 0xbf, 0x25, 0x4d, 0xbc, 0x79, 0xbf, 0x65, 0xdf, 0xef, 0x70, 0x2d, 0x98, 0xe2, 0x20, 0x94, 0x71, 0x21, 0x16, 0x91, 0x0e, 0x66, 0x60, 0x2f, 0x94, 0xb4, 0x9c};
	const uint8_t ciphertext1[16] = {0xe8, 0xe4, 0xfd, 0x22, 0xd2, 0xbc, 0x6d, 0x85, 0x74, 0x72, 0x41, 0x49, 0x41, 0x59, 0xab, 0x14};
	...
	for(int i=0; i<16; i++)
	{
		...
		{
			shared_plaintext[j*16+i] = RAND_CONST_S;
			shared_plaintext[i] ^= shared_plaintext[j*16+i];
		}
		shared_key[i] = key0[i];
		for(int j=1; j<N_SHARES; j++)
		{
			shared_key[j*16+i] = RAND_CONST_S;
			shared_key[i] ^= shared_key[j*16+i];
		}
	}

	// Encrypt
	skinny128_masked_encrypt(shared_ciphertext, shared_plaintext, shared_key, tweak0);
	...
	for(int i=0; i<16; i++)
	{
		...
		for(int j=1; j<N_SHARES; j++)
		{
			shared_ciphertext[j*16+i] = RAND_CONST_S;
	...
	// Decrypt
	skinny128_masked_decrypt(shared_plaintext, shared_ciphertext, shared_key, tweak1);
