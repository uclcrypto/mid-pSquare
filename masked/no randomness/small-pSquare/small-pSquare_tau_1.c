...
void inline SQ_dshares(const uint8_t *a, uint8_t *b);
void __attribute__((noinline)) Encrypt(uint8_t **plaintext, uint8_t **key, const uint8_t* tweak, uint8_t **ciphertext);
void __attribute__((noinline)) Decrypt(uint8_t **ciphertext, uint8_t **key, const uint8_t* tweak, uint8_t **plaintext);
...
void __attribute__((noinline)) Encrypt(uint8_t **plaintext, uint8_t **key, const uint8_t* tweak, uint8_t **ciphertext)
{
	...
	for(int i=0; i<16; i++) round_tweaks[0][i] = tweak[i];
	for(int step=0; step<steps; step++)
	{
		round_tweaks[step+1][0] = Psi0(round_tweaks[step][9]);
		round_tweaks[step+1][1] = Psi1(round_tweaks[step][5]);
		round_tweaks[step+1][2] = Psi2(round_tweaks[step][13]);
		round_tweaks[step+1][3] = Psi3(round_tweaks[step][15]);
		round_tweaks[step+1][4] = Psi4(round_tweaks[step][12]);
		round_tweaks[step+1][5] = Psi5(round_tweaks[step][7]);
		round_tweaks[step+1][6] = Psi6(round_tweaks[step][14]);
		round_tweaks[step+1][7] = Psi0(round_tweaks[step][2]);
		round_tweaks[step+1][8] = Psi1(round_tweaks[step][4]);
		round_tweaks[step+1][9] = Psi2(round_tweaks[step][6]);
		round_tweaks[step+1][10] = Psi3(round_tweaks[step][8]);
		round_tweaks[step+1][11] = Psi4(round_tweaks[step][3]);
		round_tweaks[step+1][12] = Psi5(round_tweaks[step][10]);
		round_tweaks[step+1][13] = Psi6(round_tweaks[step][1]);
		round_tweaks[step+1][14] = Psi0(round_tweaks[step][11]);
		round_tweaks[step+1][15] = Psi1(round_tweaks[step][0]);
	}
...
int main(void)
{
	...
	const uint8_t tweak0[16]      = {0x32, 0x39, 0x49, 0x5a, 0x5d, 0x2f, 0x0d, 0x20, 0x70, 0x77, 0x41, 0x5f, 0x5f, 0x5c, 0x16, 0x61};
	const uint8_t ciphertext0[16] = {0x7d, 0x76, 0x7a, 0x74, 0x06, 0x56, 0x0b, 0x41, 0x45, 0x17, 0x67, 0x5b, 0x70, 0x0f, 0x62, 0x1c};
	...
	const uint8_t tweak1[16]      = {0x2b, 0x38, 0x6b, 0x77, 0x4d, 0x3b, 0x60, 0x49, 0x4a, 0x27, 0x7a, 0x36, 0x2f, 0x2a, 0x3a, 0x52};
	const uint8_t ciphertext1[16] = {0x10, 0x6e, 0x5a, 0x3c, 0x5b, 0x54, 0x77, 0x7a, 0x26, 0x6b, 0x7b, 0x17, 0x06, 0x1d, 0x3d, 0x07};
