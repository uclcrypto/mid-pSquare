...
void __attribute__((noinline)) Encrypt(const uint8_t *plaintext, const uint8_t* key, uint8_t *ciphertext);
void __attribute__((noinline)) Decrypt(const uint8_t *ciphertext, const uint8_t* key, uint8_t *plaintext);
...
void __attribute__((noinline)) Encrypt(const uint8_t *plaintext, const uint8_t* key, uint8_t *ciphertext)
{
	...
	{
		// Add tweakey
		for(int i=0; i<16; i++) state[i] = AddModMersenne(state[i], key[i]);
	...
	for(int i=0; i<16; i++) ciphertext[i] = AddModMersenne(state[i], key[i]);
}
...
void __attribute__((noinline)) Decrypt(const uint8_t *ciphertext, const uint8_t* key, uint8_t *plaintext)
{
	...
	{
		// Add tweakey
		for(int i=0; i<16; i++) state[i] = SubModMersenne(state[i], key[i]);
	...
	// Final tweakey addition
	for(int i=0; i<16; i++) plaintext[i] = SubModMersenne(state[i], key[i]);
}
